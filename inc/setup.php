 ...
 */
function kern_setup(): void {

	// i18n. The domain is the literal 'kern' (a constant would break
	// make-pot — see bootstrap.php); the path uses KERN_DIR so it travels with a
	// re-skin. The CLI rewrites the literal when it generates a theme.
	load_theme_textdomain( 'kern', KERN_DIR . '/languages' );
	 ...
	 */
	$GLOBALS['content_width'] = (int) apply_filters( KERN_SLUG . '/content_width', 720 );
	...
	register_nav_menus(
		(array) apply_filters(
			KERN_SLUG . '/register_nav_menus',
			array(
				'primary' => esc_html__( 'Primary Navigation', 'kern' ),
				'footer'  => esc_html__( 'Footer Navigation', 'kern' ),
			)
		)
	);
	 ...
	 * @since 1.0.0
	 */
	do_action( KERN_SLUG . '/setup' );
}
add_action( 'after_setup_theme', 'kern_setup' );
 ...
 * plugin is active.
 */
function kern_woocommerce_support(): void {
...
add_action( 'after_setup_theme', 'kern_woocommerce_support' );
 ...
 */
function kern_editor_styles(): void {
	add_editor_style( array( 'assets/css/editor-style.css' ) );
}
add_action( 'after_setup_theme', 'kern_editor_styles' );
 ...
 */
function kern_comment_form_field_attributes( array $fields ): array {
	$attributes = array(
...
add_filter( 'comment_form_default_fields', 'kern_comment_form_field_attributes' );
