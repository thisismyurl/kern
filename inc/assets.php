 ...
 *
 * Portable: every handle derives from the KERN_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the KERN_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function kern_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'kern_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function kern_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( KERN_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( KERN_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'kern_preload_fonts', 1 );
