 ...
 *
 * Registers:
 *   wp kern version  — theme name and version, read from KERN_SLUG + KERN_VERSION.
 *   wp kern info     — name, version, active template, .pot presence.
 *   wp kern flush    — wp_cache_flush().
 *
 * @package kern
 */
...
class Kern_CLI_Command {
	...
	public function version(): void {
		$theme = wp_get_theme();
		\WP_CLI::log( sprintf( '%s %s', (string) $theme->get( 'Name' ), KERN_VERSION ) );
	}
	...
	public function info(): void {
		...
		$languages_dir = KERN_DIR . '/languages';
		$pot_path      = $languages_dir . '/' . KERN_SLUG . '.pot';
		$has_pot       = is_dir( $languages_dir ) && file_exists( $pot_path );

		\WP_CLI::log( 'Name:            ' . (string) $theme->get( 'Name' ) );
		\WP_CLI::log( 'Slug:            ' . KERN_SLUG );
		\WP_CLI::log( 'Version:         ' . KERN_VERSION );
		\WP_CLI::log( 'Active template: ' . $template );
		\WP_CLI::log( 'languages/.pot:  ' . ( $has_pot ? 'present' : 'missing — run: wp i18n make-pot . languages/' . KERN_SLUG . '.pot' ) );
	}
...
\WP_CLI::add_command( 'kern', 'Kern_CLI_Command' );
