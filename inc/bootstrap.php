 ...
 *
 * This is the ONE place a theme's identity lives. Change the `kern_` function
 * prefix and the KERN_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `kern_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Kern;`, on the reasoning that callbacks
 ...
 * rewrites `kern_` → `<slug>_`, `KERN_` → `<SLUG>_`, and `Kern_` →
 ...
 * stays a string LITERAL ('kern'), never the KERN_SLUG constant. `wp i18n
 ...
 */
define( 'KERN_VERSION', '1.6265.1630' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'KERN_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'KERN_URI', get_template_directory_uri() );
