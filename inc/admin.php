 ...
 * from the theme, supplied through the `KERN_SLUG . '/get_started_content'` filter
 ...
 */
define( 'KERN_WELCOME_FLAG', 'kern_welcome_notice' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
define( 'KERN_DISMISS_ACTION', 'kern_dismiss_welcome' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
define( 'KERN_GET_STARTED_SLUG', 'kern-get-started' );
 ...
 */
function kern_flag_welcome_notice(): void {
	add_option( KERN_WELCOME_FLAG, '1', '', 'no' );
}
add_action( 'after_switch_theme', 'kern_flag_welcome_notice' );
 ...
 */
function kern_clear_welcome_notice(): void {
	delete_option( KERN_WELCOME_FLAG );
}

/**
 * Register the "Get started" page under Appearance.
 *
 * The required capability comes from kern_get_onboarding_capability() — default is
 * edit_theme_options (users who can switch themes and configure global styles),
 * filterable via KERN_SLUG . '/onboarding_capability'.
 */
function kern_register_get_started_page(): void {
	add_theme_page(
		/* translators: %s: theme name. */
		sprintf( esc_html__( '%s: Get started', 'kern' ), kern_get_theme_name() ),
		kern_get_theme_name(),
		kern_get_onboarding_capability(),
		KERN_GET_STARTED_SLUG,
		'kern_render_get_started_page'
	);
}
add_action( 'admin_menu', 'kern_register_get_started_page' );
 ...
 */
function kern_get_theme_name(): string {
 ...
 */
function kern_get_onboarding_capability(): string {
	/**
	 ...
	 */
	return (string) apply_filters( KERN_SLUG . '/onboarding_capability', 'edit_theme_options' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function kern_enqueue_get_started_assets( string $hook_suffix ): void {
	if ( 'appearance_page_' . KERN_GET_STARTED_SLUG !== $hook_suffix ) {
		return;
	}

	$sheet = KERN_DIR . '/assets/css/admin-get-started.css';
	if ( ! file_exists( $sheet ) ) {
		return;
	}

	wp_enqueue_style(
		KERN_SLUG . '-get-started',
		KERN_URI . '/assets/css/admin-get-started.css',
		array(),
		(string) filemtime( $sheet )
	);
}
add_action( 'admin_enqueue_scripts', 'kern_enqueue_get_started_assets' );

/**
 * Show the dismissible welcome notice while the flag is set.
 *
 * Shown only to users who pass kern_get_onboarding_capability(), and never on the Get-started
 ...
 */
function kern_render_welcome_notice(): void {
	if ( ! current_user_can( kern_get_onboarding_capability() ) ) {
		return;
	}

	if ( '1' !== get_option( KERN_WELCOME_FLAG ) ) {
		return;
	}

	$screen = get_current_screen();
	if ( $screen instanceof \WP_Screen && 'appearance_page_' . KERN_GET_STARTED_SLUG === $screen->id ) {
		return;
	}

	$page_url = admin_url( 'themes.php?page=' . KERN_GET_STARTED_SLUG );
	$theme    = kern_get_theme_name();
	$dismiss  = wp_nonce_url( admin_url( 'admin-post.php?action=' . KERN_DISMISS_ACTION ), KERN_DISMISS_ACTION );
	?>
	<div class="notice notice-info is-dismissible kern-welcome-notice">
		<p>
			<strong>
				<?php
				/* translators: %s: theme name. */
				printf( esc_html__( 'Welcome to %s.', 'kern' ), esc_html( $theme ) );
				?>
			</strong>
			<?php esc_html_e( 'Thanks for giving it a home. A few small steps will have your site reading the way it should.', 'kern' ); ?>
		</p>
		<p>
			<a class="button button-primary" href="<?php echo esc_url( $page_url ); ?>">
				<?php
				/* translators: %s: theme name. */
				printf( esc_html__( 'Set up %s →', 'kern' ), esc_html( $theme ) );
				?>
			</a>
			<a class="kern-welcome-notice__dismiss" href="<?php echo esc_url( $dismiss ); ?>">
				<?php esc_html_e( 'Dismiss', 'kern' ); ?>
			</a>
		</p>
	</div>
	<?php
}
add_action( 'admin_notices', 'kern_render_welcome_notice' );
 ...
 */
function kern_handle_welcome_dismiss(): void {
	if ( ! current_user_can( kern_get_onboarding_capability() ) ) {
		wp_die( esc_html__( 'You do not have permission to do that.', 'kern' ) );
	}

	check_admin_referer( KERN_DISMISS_ACTION );
	kern_clear_welcome_notice();
	wp_safe_redirect( admin_url() );
	exit;
}
add_action( 'admin_post_' . KERN_DISMISS_ACTION, 'kern_handle_welcome_dismiss' );

/**
 * The Get-started page content.
 *
 * Core supplies a plain, honest default so the page is coherent out of the box.
 * A theme overrides any part of it through the `KERN_SLUG . '/get_started_content'`
 ...
 */
function kern_get_started_content(): array {
	$theme = kern_get_theme_name();

	$default = array(
		'lead'       => sprintf(
			/* translators: %s: theme name. */
			esc_html__( '%s is a free, full-site-editing theme built to get out of the way of your content. Here is how to make it yours.', 'kern' ),
			$theme
		),
		/*
		 ...
		 * echo in kern_render_get_started_page() (the 'title' at the <strong>, the
		...
		'steps'      => array(
			array(
				'title' => __( 'Choose what people land on.', 'kern' ),
				'body'  => esc_html__( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'kern' ),
			),
			array(
				'title' => __( 'Give people a way around.', 'kern' ),
				'body'  => esc_html__( 'A site without a menu is a room without doors. Open the Site Editor, edit the header, and assign your menu to Primary Navigation.', 'kern' ),
			),
			array(
				'title' => __( 'Start from a pattern, not a blank page.', 'kern' ),
				'body'  => esc_html__( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'kern' ),
			),
			array(
				'title' => __( 'Make it sound like you.', 'kern' ),
				'body'  => esc_html__( 'In the Site Editor, open Styles to change colours and typefaces. Nothing you do there can break the theme — experiment freely.', 'kern' ),
			),
		),
		'optimize'   => array(
			esc_html__( "This theme is fast by design: reading is JavaScript-free (only the breaking-news dismiss control ships a small script), self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'kern' ),
			esc_html__( 'It is built to WCAG 2.2 AA guidance — real focus outlines, a skip link, sensible heading order, and motion that respects a reduce-motion setting. Keep your own copy and images to that bar and the whole site stays welcoming.', 'kern' ),
		),
		'developers' => array(
			/* translators: %s: linked developer-guide anchor. */
			'text'  => __( 'This theme is built on Colophon, a small documented core meant to be reused. The %s walks through how to build your own theme on it.', 'kern' ),
			'url'   => apply_filters( KERN_SLUG . '/developer_guide_url', 'https://thisismyurl.com/kern/' ), // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
			'label' => __( 'developer guide', 'kern' ),
		),
	);
	 ...
	 *
	 * A theme replaces any key with its own voice. See kern_get_started_content() for
	 * the array shape.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	return (array) apply_filters( KERN_SLUG . '/get_started_content', $default ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function kern_render_get_started_page(): void {
	if ( ! current_user_can( kern_get_onboarding_capability() ) ) {
		return;
	}
	...
	$theme   = kern_get_theme_name();
	$content = kern_get_started_content();
	...
	?>
	<div class="wrap kern-get-started">

		<h1>
			<?php
			/* translators: %s: theme name. */
			printf( esc_html__( '%s: Get started', 'kern' ), esc_html( $theme ) );
			?>
		</h1>

		<?php if ( ! empty( $content['lead'] ) ) : ?>
			<p class="kern-get-started__lead"><?php echo wp_kses( $content['lead'], $cl_inline ); ?></p>
		<?php endif; ?>

		<?php if ( ! empty( $content['steps'] ) && is_array( $content['steps'] ) ) : ?>
			<h2><?php esc_html_e( 'Set up', 'kern' ); ?></h2>
			<ol class="kern-get-started__steps">
		...
		<?php if ( ! empty( $content['optimize'] ) && is_array( $content['optimize'] ) ) : ?>
			<h2><?php esc_html_e( 'Optimize', 'kern' ); ?></h2>
		...
		<?php if ( ! empty( $content['developers']['text'] ) ) : ?>
			<h2><?php esc_html_e( 'For developers', 'kern' ); ?></h2>
			<p>
				<?php
